...
where ' . $securedSql : '';
			...
			if($this->_queryId){
				while($row = mysql_fetch_object($this->_queryId)){
					...
					foreach($properties as $property){
						...
						if($property->isPublic() && array_key_exists($name, $attributes)){
		...
		public function useDatabase($databaseName){
			...
			if($databaseName != null && $this->_connectionId != null){
				try{
					mysql_select_db($databaseName, $this->_connectionId);
					$this->setError(null);
					...
					if($property->isPublic()){
						...
						if($should_ad){
							...
							if(method_exists($obj, 'willAddFieldToSaveList')){
								$value = $obj->willAddFieldToSaveList($name, $value);
							}
							$value = $this->typeIt($value);
							$values[] = $value;
							if(method_exists($obj, 'didAddFieldToSaveList')){
								$obj->didAddFieldToSaveList($name, $value);
							}
						}
					}
					$should_ad = false;
					...
					if($property->isPublic()){
						...
						if($name != $key
							...
							if(method_exists($obj, 'willAddFieldToUpdateList')){
								$value = $obj->willAddFieldToUpdateList($name, $value);
							}
							$values[] = $name . '=' . $value;
							if(method_exists($obj, 'didAddFieldToUpdateList')){
								$obj->willAddFieldToUpdateList($name, $value);
							}
