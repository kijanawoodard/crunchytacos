<?php
	...
	class Publisher extends Object{
		...
		public function getTableName($config = null){
			...
			return $config->prefix . 'publishers';
		}
		public static function delete(Publisher $publisher){
			$config = new AppConfiguration();
			$db = Factory::get($config->db_type, $config);
			return $db->delete(null, $publisher);
		}
		public static function save(Publisher $publisher){
			$errors = self::canSave($publisher);
			$config = new AppConfiguration();
			if(count($errors) == 0){
				$db = Factory::get($config->db_type, $config);
				$db->save(null, $publisher);
				self::notify('didSavePublisher', $publisher, $publisher);
			}
			return $errors;
		}
		...
		public static function findAll(){
			$config = new AppConfiguration();
			$db = Factory::get($config->db_type, $config);
			$list = $db->find(new All(null, null, 0, null), new Publisher());
			return $list;
		}
		...
		public static function findById($id){
			$config = new AppConfiguration();
			$db = Factory::get($config->db_type, $config);
			$list = $db->find(new ById($id), new Publisher());
			return $list;
		}
		...
		public static function findByEmail($email){
			$config = new AppConfiguration();
			$db = Factory::get($config->db_type, $config);
			$list = $db->find(new ByAttribute('email', $email, 1, null), new Publisher());
			return $list;
		}
		...
		public static function findByPublicKey($public_key){
			$config = new AppConfiguration();
			$db = Factory::get($config->db_type, $config);
			$list = $db->find(new ByAttribute('public_key', urlencode($public_key), 1, null), new Publisher());
			return $list;
		}
		
		public static function canSave(Publisher $publisher){
			$errors = array();
		...
		public function install(Configuration $config){
			$message = '';
			$db = Factory::get($config->db_type, $config);
			error_log('installing publishers');
			try{
				...
				$table->addColumn('body', 'text', array('is_nullable'=>false));
