...
ytvb.selected = new Array();
ytvb.videoWasSelected = function(e){
	var links = $$('#' + ytvb.VIDEO_LIST_TABLE_CONTAINER_DIV + ' a');
	links.each(function(item){
		item.removeClass('selected');
	});
	if(ytvb.selected.length > 1){
		ytvb.selected.shift();
	}
	ytvb.selected.push(e.target.parentNode.rel);
	ytvb.selected.each(function(key){
		$$('#' + ytvb.VIDEO_LIST_TABLE_CONTAINER_DIV + ' a[rel=' + key + ']')[0].addClass('selected');
	});
}
...
ytvb.listVideosCallback = function(data) {
  ...
  var resultsTableContainer = document.getElementById(
	...
	resultsTableContainer.innerHTML = null;
	  /*
// Deletes and re-adds the results table from container
  ...
  for (var i = 0, entry; entry = data.feed.entry[i]; i++) {
	...
	container.setAttribute('clss', ytvb.VIDEO_LIST_CSS_CLASS);
	resultsTableContainer.appendChild(container);
	var key = '';
	for(var i = 0, entry; entry = data.feed.entry[i]; i++){
		key = ytvb.queryPathToObject(entry.media$group.media$player[0].url).v;
		container.innerHTML += '<li><a rel="' + key + '" href="#' + key + '" title="' + entry.media$group.media$title.$t +'"><img src="' + entry.media$group.media$thumbnail[0].url + '" rel="' + key + '" /></a></li>';
	}
};
ytvb.queryPathToObject = function(url){
